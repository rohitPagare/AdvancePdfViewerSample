...
import com.orhotech.advancepdfviewer.listener.OnDrawListener;
import com.orhotech.advancepdfviewer.listener.OnErrorListener;
import com.orhotech.advancepdfviewer.listener.OnLoadCompleteListener;
import com.orhotech.advancepdfviewer.listener.OnPageChangeListener;
import com.orhotech.advancepdfviewer.listener.OnPageErrorListener;
import com.orhotech.advancepdfviewer.listener.OnPageScrollListener;
import com.orhotech.advancepdfviewer.listener.OnRenderListener;
...
import com.orhotech.advancepdfviewer.source.AssetSource;
import com.orhotech.advancepdfviewer.source.ByteArraySource;
import com.orhotech.advancepdfviewer.source.DocumentSource;
import com.orhotech.advancepdfviewer.source.FileSource;
import com.orhotech.advancepdfviewer.source.InputStreamSource;
import com.orhotech.advancepdfviewer.source.UriSource;
import com.orhotech.advancepdfviewer.util.ArrayUtils;
import com.orhotech.advancepdfviewer.util.Constants;
import com.orhotech.advancepdfviewer.util.MathUtils;
...
public class PDFView extends RelativeLayout {
    ...
    private void load(DocumentSource docSource, String password, OnLoadCompleteListener onLoadCompleteListener, OnErrorListener onErrorListener, int[] userPages) {
        ...
        decodingTask = new DecodingTask(docSource, password, this, pdfiumCore, firstPageIdx);
    ...
    public boolean canScrollHorizontally(int direction) {
        if (swipeVertical) {
            if (direction < 0 && currentXOffset < 0) {
                ...
                return direction > 0 && currentXOffset + toCurrentScale(optimalPageWidth) > getWidth();
        } else {
            if (direction < 0 && currentXOffset < 0) {
                return true;
    ...
    public boolean canScrollVertically(int direction) {
        if (swipeVertical) {
            ...
            if (direction < 0 && currentYOffset < 0) {
                ...
                return direction > 0 && currentYOffset + toCurrentScale(optimalPageHeight) > getHeight();
        }
    }
    ...
    private float calculateCenterOffsetForPage(int pageNb) {
        if (swipeVertical) {
            ...
            imageY += (float) getHeight() / 2 - optimalPageHeight / 2;
            return imageY;
        } else {
            float imageX = -(pageNb * optimalPageWidth + pageNb * spacingPx);
            imageX += (float) getWidth() / 2 - optimalPageWidth / 2;
            return imageX;
        }
    }
    ...
    void loadPageByOffset() {
        ...
        float spacingPerPage = spacingPx - ((float) spacingPx / getPageCount());
        if (swipeVertical) {
